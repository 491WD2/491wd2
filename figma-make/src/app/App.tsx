...
import {
  ...
  Image, BookOpen, RefreshCw, Settings, Plus, ScanLine,
  Bell, Search, X, Check, MoreHorizontal, ChevronRight,
  Trash2, AlertCircle, Cloud, Menu, Filter, ChevronDown,
  Users, Thermometer, Wind, Droplets, Zap, Star, Tag,
  Edit2, Archive, Camera, ClipboardList, ShieldAlert,
  PawPrint, CalendarCheck, Layers, Bookmark, ListTodo
} from 'lucide-react';
...
type View =
  ...
  | 'cleaning' | 'emergency' | 'pets' | 'subscriptions' | 'projects'
  | 'photos' | 'planner' | 'routines' | 'settings';
...
const FAMILY_MEMBERS = [
  { id: 'hershel',  name: 'Hershel',  color: '#4F46E5', bg: '#EEF2FF' },
  { id: 'lorraine', name: 'Lorraine', color: '#DB2777', bg: '#FCE7F3' },
  { id: 'stella',   name: 'Stella',   color: '#D97706', bg: '#FEF3C7' },
  { id: 'nox',      name: 'Nox',      color: '#059669', bg: '#D1FAE5' },
  { id: 'jeremiah', name: 'Jeremiah', color: '#7C3AED', bg: '#EDE9FE' },
  { id: 'selena',   name: 'Selena',   color: '#EA580C', bg: '#FFEDD5' },
];
...
const PROJECTS = [
  { id: '1', name: 'Backyard garden bed', status: 'In progress', progress: 60, due: 'Sep 15',  color: '#10B981', tasks: 4 },
  { id: '2', name: 'Garage declutter',   status: 'Planning',    progress: 20, due: 'Aug 31',  color: '#F59E0B', tasks: 7 },
  { id: '3', name: 'Bathroom repaint',   status: 'Done',        progress: 100,due: 'Jul 28',  color: '#4F46E5', tasks: 5 },
  { id: '4', name: 'New filing system',  status: 'In progress', progress: 40, due: 'Sep 30',  color: '#8B5CF6', tasks: 3 },
];

const ROUTINES = [
  { id: '1', name: 'Morning routine',   time: '7:00 AM', days: ['M','T','W','Th','F'],       steps: 6, color: '#F59E0B' },
  { id: '2', name: 'School drop-off',   time: '8:15 AM', days: ['M','T','W','Th','F'],       steps: 3, color: '#4F46E5' },
  { id: '3', name: 'After-school',      time: '3:30 PM', days: ['M','T','W','Th','F'],       steps: 4, color: '#10B981' },
  { id: '4', name: 'Evening wind-down', time: '8:30 PM', days: ['M','T','W','Th','F','Sa','Su'], steps: 5, color: '#8B5CF6' },
  { id: '5', name: 'Weekend chores',    time: '10:00 AM',days: ['Sa'],                        steps: 8, color: '#EA580C' },
];
...
function SubscriptionsView() {
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <p className="text-sm text-stone-500 mb-5">Monthly total: <span className="font-semibold text-stone-800">${monthlyTotal.toFixed(2)}</span></p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {PROJECTS.map(proj => (
          <Card key={proj.id} className="p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: proj.color + '18' }}>
                <Layers size={18} style={{ color: proj.color }} />
              </div>
              <Badge color={proj.status === 'Done' ? '#10B981' : proj.status === 'In progress' ? proj.color : '#94a3b8'} light>
                {proj.status}
              </Badge>
            </div>
            <h3 className="font-semibold text-stone-900 mb-1">{proj.name}</h3>
            <div className="text-xs text-stone-400 mb-3">Due {proj.due} · {proj.tasks} tasks</div>
            <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all" style={{ width: `${proj.progress}%`, backgroundColor: proj.color }} />
            </div>
            <div className="text-xs text-stone-400 mt-1.5">{proj.progress}% complete</div>
          </Card>
        ))}
      </div>
    </div>
...
function PhotosView() {
  const PHOTOS = [
    { id: '1', url: 'https://images.unsplash.com/photo-1511895426328-dc8714191011?w=400&h=400&fit=crop&auto=format', alt: 'Family at the park', label: 'Park day', date: 'Aug 3' },
    { id: '2', url: 'https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=400&h=400&fit=crop&auto=format', alt: 'Family dinner', label: 'Sunday dinner', date: 'Jul 28' },
    { id: '3', url: 'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=400&h=400&fit=crop&auto=format', alt: 'Home living room', label: 'New couch!', date: 'Jul 20' },
    { id: '4', url: 'https://images.unsplash.com/photo-1587560699334-cc4ff634909a?w=400&h=400&fit=crop&auto=format', alt: 'Dog Luna', label: 'Luna at the vet', date: 'Jul 18' },
    { id: '5', url: 'https://images.unsplash.com/photo-1534353436294-0dbd4bdac845?w=400&h=400&fit=crop&auto=format', alt: 'Garden', label: 'Garden progress', date: 'Jul 15' },
    { id: '6', url: 'https://images.unsplash.com/photo-1484981138541-3d074aa97716?w=400&h=400&fit=crop&auto=format', alt: 'Cooking together', label: 'Taco night', date: 'Jul 10' },
  ];
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-white bg-pink-500 hover:bg-pink-600 transition-colors">
          <Camera size={15} /> Upload
        </button>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {PHOTOS.map(photo => (
          <div key={photo.id} className="group relative rounded-2xl overflow-hidden bg-stone-200 aspect-square cursor-pointer">
  ...
  return (
    <div className="p-6 max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-white bg-cyan-500 hover:bg-cyan-600 transition-colors">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-1 mt-2">
                {['M','T','W','Th','F','Sa','Su'].map(d => (
                  <span key={d} className={`text-xs px-1.5 py-0.5 rounded-md font-medium transition-colors
                    ${r.days.includes(d) ? 'text-white' : 'text-stone-300 bg-stone-100'}`}
                    style={r.days.includes(d) ? { backgroundColor: r.color } : {}}>
                    {d}
                  </span>
                ))}
              </div>
            </div>
            <button className="text-stone-300 hover:text-stone-600 p-1 transition-colors">
              <Edit2 size={15} />
            </button>
          </Card>
        ))}
...
function SettingsView() {
  const SECTIONS = [
    ...
    {
      title: 'App', items: [
        { label: 'Kiosk mode', sub: 'Surface Pro display', icon: Layers },
...
export default function App() {
  ...
  const updatePantryStock = (id: string, qty: number) =>
    setPantryItems(prev => prev.map(i => {
      if (i.id !== id) return i;
      const pct = qty / i.max;
      const status = qty <= 0 ? 'out' : pct < 0.25 ? 'low' : pct < 0.5 ? 'ok' : 'good';
      return { ...i, qty, status };
    }));
